      ...
      <div className="container mx-auto px-5 md:px-8 lg:px-4">
        <ol className="flex items-center gap-1.5 py-4 border-b border-navy-30">
          ...
          {pages?.map((page, index) => {
            ...
            return (
              <li key={pageData.id} className="flex items-center gap-1.5">
                <ChevronSeparator />
                {isLast ? (
                  <span className="text-[14px] font-medium leading-[22px] tracking-[-0.28px] text-navy-500">
                    {pageData.title}
                  </span>
                ) : (
                  <Link
                    href={`/${pageData.slug}`}
                    className="text-[14px] font-medium leading-[22px] tracking-[-0.28px] text-orange hover:text-navy-1000 transition-colors"
                  >
                    {pageData.title}
                  </Link>
                )}
